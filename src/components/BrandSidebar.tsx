...
const baseMainItems: NavItem[] = [
  { title: "Overview", url: "/brand/dashboard", icon: LayoutGrid },
  { title: "Campaigns", url: "/brand/campaigns", icon: Megaphone },
  { title: "Creators", url: "/brand/creators", icon: Users },
  { title: "Invites & Applications", url: "/brand/applications", icon: Mail },
  { title: "Reports", url: "/brand/reports", icon: BarChart3 },
  { title: "Payments", url: "/brand/payments", icon: CreditCard },
  { title: "Social Listening", url: "/brand/social-listening", icon: Radio },
  { title: "Brand Profile", url: "/brand/profile", icon: Building2 },
];

const bottomItems: NavItem[] = [
  { title: "Settings", url: "/brand/settings", icon: Settings },
  { title: "Help", url: "/brand/help", icon: HelpCircle },
];

// Persist pin state in localStorage
const SIDEBAR_PIN_KEY = "brand_sidebar_pinned";
...
export function BrandSidebar() {
  ...
  const { currentUserRole, loading: roleLoading, isOwner, isAdmin } = useCachedBrandRole();
  ...
  // Create mainItems with role-based filtering and dynamic badge
  const mainItems = useMemo<NavItem[]>(() => {
    // Show loading skeleton if still loading
    if (roleLoading) return baseMainItems;

    // Filter items based on role
    let filteredItems = baseMainItems.filter(item => {
      // Finance role: only see Payments, Reports
      if (currentUserRole === 'finance') {
        return ['/brand/payments', '/brand/reports', '/brand/dashboard'].includes(item.url);
      }
      
      // Campaign Manager: see Overview, Campaigns, Creators, Invites & Applications, but not Payments
  ...
  }, [pendingCount, currentUserRole, roleLoading]);
  ...
  return (
    <motion.aside
      ...
      style={{
        background: "linear-gradient(165deg, hsl(239 62% 50%) 0%, hsl(280 70% 45%) 50%, hsl(330 77% 50%) 100%)",
      }}
    ...
    >
      ...
      <div className="p-5 flex items-center justify-between shrink-0">
        <div
          ...
          onClick={() => handleNavigation("/brand/dashboard")}
        >
        ...
        {isExpanded && (
          ...
          >
            <div className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-white/10 border border-white/10">
              ...
              <span className="text-sm font-medium text-white/80">Brand Dashboard</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <nav className="flex-1 min-h-0 py-3 overflow-y-auto scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent hover:scrollbar-thumb-white/30 transition-colors">
        {/* Loading skeleton for role-based nav */}
        {roleLoading ? (
          <div className="space-y-2 px-3">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="flex items-center gap-4 py-3.5 px-4 min-h-[52px]">
                <Skeleton className="w-5 h-5 rounded bg-white/20" />
                {isExpanded && <Skeleton className="h-4 w-24 bg-white/20" />}
              </div>
            ))}
          </div>
        ) : (
